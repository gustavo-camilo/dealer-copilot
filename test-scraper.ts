...
const TEST_URLS = [
  ...
  'https://www.carvana.com/cars', // Another common one
];
...
async function testPythonScraper(url: string): Promise<ScraperResult> {
...
async function testPlaywrightScraper(url: string): Promise<ScraperResult> {
  const startTime = Date.now();
...
async function testHealthEndpoints() {
  ...
  try {
    ...
    console.log(`❌ Python Scraper Health: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  try {
    ...
    console.log(`❌ Playwright Scraper Health: ${error instanceof Error ? error.message : String(error)}`);
  }
}

function printResults(results: ScraperResult[]) {
  console.log('\n=== TEST RESULTS ===\n');

...
interface VehicleCommentSectionProps {
  vinScanId: string;
  tenantId: string;
  currentAuctionUrl?: string;
  onAuctionUrlChange?: (url: string) => void;
  onSaveAuctionUrl?: () => void;
}

export function VehicleCommentSection({
  vinScanId,
  tenantId,
  currentAuctionUrl = '',
  onAuctionUrlChange,
  onSaveAuctionUrl
}: VehicleCommentSectionProps) {
  const { user } = useAuth();
          ...
          <div className="space-y-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 pr-10 border border-gray-300 dark:border-navy-600 rounded-md bg-white dark:bg-navy-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
            ...
            {onAuctionUrlChange && onSaveAuctionUrl && (
              <div data-auction-url-section>
                ...
                <div className="flex gap-2">
                  <input
                    type="url"
                    value={currentAuctionUrl}
                    onChange={(e) => onAuctionUrlChange(e.target.value)}
                  ...
                  <button
                    ...
                    disabled={isSaving}
                    className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    Save URL
                  </button>
                </div>

...
import type { VINDecodedData, ParsedVehicle } from './types.ts';
 ...
 * @returns Decoded vehicle data or null if invalid/failed
 */
export async function decodeVIN(vin: string): Promise<VINDecodedData | null> {
  ...
  try {
    ...
    const bodyType = getValueByName('Body Class');
    const engineType = getValueByName('Engine Model');
    const driveType = getValueByName('Drive Type');
 ...
 * Converts "TOYOTA" -> "Toyota", "F-150" -> "F-150"
 */
 ...
 *
 * @param vehicle - Partial vehicle data
 * @returns Vehicle with missing fields filled from VIN
 */
